...
class Login extends CI_Controller{
	public function __construct(){
		...
		$this->load->model('M_pemilik', 'm_pemilik');
	}
	...
	public function proses_login(){
		...
		elseif($this->input->post('role') === 'pemilik') $this->_proses_login_pemilik($this->input->post('username'));
		else {
			?>
			<script>
	...
	protected function _proses_login_pemilik($username){
		$get_pemilik = $this->m_pemilik->lihat_username($username);
		if($get_pemilik){
			if($get_pemilik->password == $this->input->post('password')){
				$session = [
					'kode' => $get_pemilik->kode,
					'nama' => $get_pemilik->nama,
					'username' => $get_pemilik->username,
					'password' => $get_pemilik->password,
					'role' => $this->input->post('role'),
					'jam_masuk' => date('H:i:s')
				];
